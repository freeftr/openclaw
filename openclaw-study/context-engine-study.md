...
**TL;DR (한 문장):** compaction은 메시지를 *지우는* 게 아니라, append-only DAG에 **마커 노드**를 심어 조립 시점에 옛 구간을 건너뛰게 만드는 *뷰 변환*이다.

**말할 순서 (7비트):**
1. **착시 깨기** — 저장 진실 / 활성 경로 / 모델 뷰, 3층 분리 (§0)
2. **DAG 구조** — `parentId` 1개 + `leaf` 마커로 10종 노드가 트리를 이룬다 (§1)
3. **저장** — JSONL append-only, 한 줄도 안 지움 (§2)
4. **★핵심** — compaction 3단계(prepare→compact→append) + **before/after DAG 그림** (§3)
5. **가지 요약**도 같은 원리 — 또 다른 우회 노드 (§4)
6. **조립이 실제로 접는 곳** — context-engine은 껍데기, 진짜는 buildSessionContext (§5)
7. **재개 vs fork** (§6) → "하나의 원시요소가 다섯 편집을 표현" 한 문장 마무리 (§7)

**예상 질문 & 답 (미리 준비):**
| 질문 | 답 | 근거 |
|---|---|---|
| "그럼 디스크가 계속 커지나?" | 네, **단조 증가**. 토큰은 줄어도 파일은 안 줄어든다 | §0 평가 |
| "요약이 부정확하면 옛 대화 못 살리나?" | 원본이 그대로 남아 **fork·재개로 접근 가능** (지운 게 아님) | §3 |
| "context-engine이 이걸 하나?" | 기본 경로에선 **껍데기**. 실제 투영은 buildSessionContext/attempt.ts | §5 |
| "토큰은 어떻게 세나?" | 실 토크나이저 아님, **chars/token 휴리스틱**(text=4, image=2000) | §5 |
| "compaction 여러 번이면?" | 경로에 남아도 **최신 마커 하나만 유효**, 나머지는 조립이 덮어씀 | §3 함정 |

**핵심 그림 하나만 보여준다면:** §3의 before/after DAG ASCII (파일엔 u1이 남아있는데 모델엔 안 보이는 장면).
